...
const Navbar = () => {
  ...
  return (
    <div>
      <nav className='flex gap-2.5 lg:gap-0 justify-between items-center bg-[rgba(0,0,0,1)] w-full h-[96px] px-[1.5rem] lg:px-[4rem] fixed z-50'>
          <div>
          <img src={JuneLogo} alt="" className="w-[120px] h-[76]  relative"/>
          </div>

          <ul className="hidden lg:flex lg:justify-center lg:gap-[38px] lg:text-white lg:text-[15.25px] lg:leading-[28px]">
            {navItems.map((item) => (
              <li
                key={item.name}
